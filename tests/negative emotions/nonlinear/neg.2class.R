# Figure XX of manuscript - nonlinear effect of NE on HP

rm(list = ls(all = TRUE)) #clear workspace
#setwd("C:/Documents and Settings/jolynn/Desktop/Fall 2010/CI auto/examples/negative emotions/nonlinear")
#setwd("C:/Documents and Settings/Diane Losardo/Desktop/Quant Research/mixtureproj/workingcode")
#setwd("E:/C drive/Academic Courses/11 Fall 2010/CI auto/examples/negative emotions/nonlinear")
setwd("C:/Documents and Settings/pek/Desktop/CEauto/example/negative emotions/nonlinear")


nclass <- 2 #user input
nparm <- 25 #user input - can be used to check whether acov has correct dimensions
alpha<- 0.1 #user defined
...
beta_loc <- c(17,22) 
psi_loc <- c(18,19,23,24)  #psi1, psi2 by class
...
plot(Ksi,Eta,type='n',xlab="Negative Emotions", ylab="", 
	cex.lab=1, cex.axis=1,yaxt="n")
...
y1 <- -0.25+exp(-0.14*x)
lines(x,y1,lty=1,col="grey",lwd=2)
...
#for (i in 1:n){
#	ytemp = syall[,i]
#	lines(x,ytemp,col="grey")
#}


#legend("topleft", legend=c('Linear Function', 'Delta Method 95% Confidence Interval',
#	'Bootstrap 95% Confidence Interval'), lwd=c(2,1,1), lty=c(0,0,0),
#	pch=c(16,1,4),col=c(1,1,1),bty="n", cex=.8
#	)
...
write.table(out, file='neg.2class.dat', append=FALSE, quote=FALSE,
